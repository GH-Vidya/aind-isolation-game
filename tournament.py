...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("\n{:^9}{:^13}".format("Match #", "Opponent") + ''.join(['{:^20}'.format(x[1].name) for x in enumerate(test_agents)]))
    print("{:^9}{:^13} ".format("", "") +  ' '.join(['{:^5}| {:^5}| {:^5}'.format("Won", "Lost", "Time") for x in enumerate(test_agents)]))
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        print(' ' + ' '.join([
            '{:^5}| {:^5}| {:^5}'.format(
                round_totals[i],round_totals[i+1], round(Decimal(elapsed),2)
    ...
    print('{:^9}{:^13}'.format("", "Win Rate:") +
        ''.join([
            '{:^13}| {:^8}'.format(
                "{:.1f}%".format(100 * total_wins[x[1].player] / total_matches), "{:.2f}".format(timer / 7)
